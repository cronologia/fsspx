...
test('renderPage with viz keys renders both sections and their nav links', () => {
  ...
  assert.match(html, /<section id="lineage">/);
  assert.match(html, /<section id="branch-timeline">/);
  // fsspx section order (#28): the branch timeline opens the page — it renders
  // before About/Chronology; then lineage, then figures.
  const order = ['id="branch-timeline"', 'id="about"', 'id="chronology"', 'id="lineage"', 'id="figures"'].map((s) => html.indexOf(s));
...
test('renderPage renders the numbers chart section, its nav link, and section order', () => {
  ...
  assert.match(html, /<section id="numbers-chart">/);
  // fsspx section order (#28): branch timeline first, then chronology,
  // lineage, the numbers chart and figures.
  const order = ['id="branch-timeline"', 'id="chronology"', 'id="lineage"', 'id="numbers-chart"', 'id="figures"'].map((s) => html.indexOf(s));
  assert.deepEqual([...order].sort((x, y) => x - y), order);
});
